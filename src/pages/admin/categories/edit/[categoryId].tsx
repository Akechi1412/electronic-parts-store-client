...
export default function AdminEditCategoryPage() {
  ...
  const [filteredList, setFilteredList] = useState<CategoryData[]>([]);
  const [categoryList, setCategoryList] = useState<CategoryData[]>([]);
  const [newCategory, setNewCategory] = useState(false);
  const [emptyName, setEmptyName] = useState(false);
  const [thumbnail, setThumbnail] = useState('');
  const [oldCategoryData, setOldCategoryData] = useState<CategoryData>();
  ...
  function handleResetClick() {
    setName(oldCategoryData?.name as string);
    setThumbnail(' ');
    setSelectedFile(undefined);
    setParent({
      id: oldCategoryData?.parent_id || 0,
      name: oldCategoryData?.parent_name as string,
    });
  }
  ...
  async function handleSubmitClick() {
    ...
    try {
      ...
      await categoriesApi.update(Number(query.categoryId), {
        name,
  ...
  useEffect(() => {
    setThumbnail(oldCategoryData?.thumbnail as string);
  ...
  useEffect(() => {
    (async () => {
      try {
        if (!query || !query?.categoryId) return;
        const { data } = await categoriesApi.getById(Number(query?.categoryId));
        setOldCategoryData(data);
        setThumbnail(data?.thumbnail);
        setName(data?.name);
        setParent({ id: data?.parent_id, name: data?.parent_name });
      } catch (error: any) {
        const errorMessage = error.response?.data?.message || error.message;
        Swal.fire({
          icon: 'error',
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const filteredData = categoriesData.filter((item: any) => item.id != query.categoryId);
        setCategoryList([{ id: 0, name: 'Không có' }, ...filteredData]);
      } catch (error: any) {
        const errorMessage = error.response?.data?.message || error.message;
        Swal.fire({
          icon: 'error',
  ...
  }, [textFilter, handleFilterDebounce]);
        ...
        {loading ? (
          ...
          <div className="mt-7">
            <div className="flex justify-between items-end">
              <h2 className="text-[28px] font-semibold capitalize">Sửa thông tin danh mục</h2>
